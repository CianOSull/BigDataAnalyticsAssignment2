...
def my_main(spark, my_dataset_dir, top_n_bikes):
    ...
    # Group the rows by bike_id, then count them and total the trip duration column for each
    groupby_aggDF = inputDF.groupBy(["bike_id"]).agg( {"trip_duration" : "sum", "bike_id" : "count"} )

    # Rename the columns to be clearer
    groupby_aggDF = groupby_aggDF.withColumnRenamed("sum(trip_duration)", "totalTime").withColumnRenamed("count(bike_id)","numTrips")
    
    # Sort by total time in descending
    sortedDF = groupby_aggDF.sort(groupby_aggDF.totalTime.desc())
    
    # Get the first 10 rows
    solutionDF = sortedDF.limit(top_n_bikes)
    
    # Put in the same order as the results in the example solution
    solutionDF = solutionDF.select(solutionDF["bike_id"], solutionDF["totalTime"], solutionDF["numTrips"])
